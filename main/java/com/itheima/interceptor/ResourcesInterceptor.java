...
public class ResourcesInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws
            Exception {
        Administrator administrator = (Administrator) request.getSession().getAttribute("USER_SESSION");
        //获取请求的路径
        String uri = request.getRequestURI();
        //如果用户是已登录状态，判断访问的资源是否有权限
        if (administrator != null) {
            //如果是管理员，放行
            if ("ADMIN".equals(administrator.getRole())) {
                return true;
            }
            //如果是普通用户
            else if (!"ADMIN".equals(administrator.getRole())) {
